...
function inverse(data) {
  return {
    from: data.map(i => Number(i[0])),
    open: data.map(i => Number(i[1])),
    high: data.map(i => Number(i[2])),
    low: data.map(i => Number(i[3])),
    close: data.map(i => Number(i[4])),
    volume: data.map(i => Number(i[5])),
    trades: data.map(i => Number(i[8]))
  };
}
